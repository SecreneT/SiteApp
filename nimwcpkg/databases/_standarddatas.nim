...
proc standardDataSettings*(db: DbConn, dataStyle = cssBootstrap) =
  ## Settings
...
proc standardDataFrontpage*(db: DbConn, dataStyle = cssBootstrap) =
  ## Frontpage
  ...
  if dataStyle == cssWater:
    ...
    discard insertID(db, sqlFrontpageData, "1", "2", "frontpage", "Frontpage", frontpage, "1", "1", "1", "SecreneT", "A Knowledge Documentation based WebsiteApp with a Blog of Contents and Services", "website,blog")
...
proc createStandardData*(db: DbConn, dataStyle = cssBootstrap, confirm = false) {.discardable.} =
  ## Insert basic data
